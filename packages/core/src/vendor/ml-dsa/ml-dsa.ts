 ...
 * Upstream file  : src/ml-dsa.ts
 * Upstream repo  : https://github.com/paulmillr/noble-post-quantum
 * License        : MIT (c) 2024 Paul Miller — full text in ../ml-kem/NOTICE.md
 * Vendored on    : 2026-09-08
 * sha256 of the pristine copy, before local edits:
 *   7c54716e7e2e7983b2132b0d3ebd04c5c3d031efffc7c17d15db75245ead9992
 *
 ...
 * cannot reach them. Vendoring is the only way a FIPS 204 correction ships. See
 * docs/compliance/FIPS-204-MATRIX.md §1.3.
 *
 * Shared dependencies: `_crystals.ts` and `utils.ts` are NOT duplicated here. This file
 * imports them from ../ml-kem/, where they were already vendored for ML-KEM — they are
 * whole-file copies of upstream's shared helpers and serve every algorithm. All 19 symbols
 * this file needs were verified present in that copy before wiring (note `abytes` is
 ...
 *   2. F204-13: GAMMA2_1/GAMMA2_2, decompose(), HINT_M and Power2Round() no longer use
 *      `Math.floor(x / y)`, which is IEEE-754 double division (FIPS 204 §3.6.4).
 *   3. F204-10: internal.verify() zeroizes its intermediates via try/finally, where
 *      upstream had no cleanBytes call on that path at all (FIPS 204 §3.6.3).
 *   4. Added the `@ts-nocheck` directive on line 1 — see below.
 *
 * Type checking: this repo compiles with `noUncheckedIndexedAccess`, which upstream does
 * not use. As with the vendored ML-KEM files, the resulting errors are all `T | undefined`
 * from array indexing inside hot lattice arithmetic and none indicates a real defect.
 * `@ts-nocheck` suppresses checking of this file only; the annotated exports still present
 * correct types to importers. The cost is stated plainly: this file is not type-checked by
 * the repo's gate. It is covered by ./__tests__/ and, once wired, by the ACVP vectors.
 */
/**
...
import { genCrystals, type XOF, XOF128, XOF256 } from '../ml-kem/_crystals.js';
import {
  abytes,
  type BytesCoderLen,
  checkHash,
...
// LOCAL MODIFICATION (F204-13). Upstream: `Math.floor((Q - 1) / 88) | 0` and
// `Math.floor((Q - 1) / 32) | 0`. In JavaScript `/` is always IEEE-754 double division and
// `Math.floor` does not change the arithmetic performed, so both were floating-point
// operations. FIPS 204 §3.6.4: "Implementations of ML-DSA shall not use floating-point
// arithmetic". Evaluated once at module load, so the integer forms cost nothing.
// 88 is not a power of two -> BigInt exact division. 32 is -> §3.6.4's own guidance applies:
// "If y is a power of two, it may be more efficient to use bit shift operations than integer
// division." Both verified against the originals in ./__tests__/rounding-equivalence.test.ts.
const GAMMA2_1 = Number(BigInt(Q - 1) / 88n) | 0;
const GAMMA2_2 = ((Q - 1) >>> 5) | 0;
...
function getDilithium(opts_: TArg<DilithiumOpts>): TRet<DSA> {
  ...
  // LOCAL MODIFICATION (F204-13). `2 * GAMMA2` is 190464 (ML-DSA-44) or 523776 (ML-DSA-65/87)
  // — neither is a power of two, so the shift shortcut §3.6.4 mentions does not apply and the
  // division is done in BigInt, which has no floating-point representation. Hoisted here so
  // the per-call cost is one BigInt allocation rather than a conversion of the divisor too.
  const TWO_GAMMA2 = BigInt(2 * GAMMA2);
  ...
  const decompose = (r: number) => {
    ...
    // LOCAL MODIFICATION (F204-13). Upstream: `Math.floor((rPlus - r0) / (2 * GAMMA2)) | 0`.
    // `rPlus - r0` is by construction an exact multiple of 2*GAMMA2 and is non-negative, so
    // BigInt's truncating division reproduces the value exactly. Verified over the complete
    // domain (rPlus in [0, Q) for both GAMMA2 values) in
    // ./__tests__/rounding-equivalence.test.ts — 0 discrepancies, no sampling.
    const r1 = Number(BigInt(rPlus - r0) / TWO_GAMMA2) | 0;
  ...
  // LOCAL MODIFICATION (F204-13): BigInt exact division, evaluated once per parameter set.
  const HINT_M = Number(BigInt(Q - 1) / TWO_GAMMA2);
  ...
  const Power2Round = (r: number) => {
    ...
    // LOCAL MODIFICATION (F204-13). Upstream: `Math.floor((rPlus - r0) / 2 ** D) | 0`.
    // Here the divisor 2^13 *is* a power of two, so FIPS 204 §3.6.4's own guidance applies —
    // "If y is a power of two, it may be more efficient to use bit shift operations than
    // integer division" — and `>>> D` is both integer-only and faster than BigInt would be.
    // `rPlus - r0` is non-negative and below 2^31 (rPlus < Q, r0 >= -2^12), so the unsigned
    // shift is exact. Verified over the complete domain rPlus in [0, Q) in
    // ./__tests__/rounding-equivalence.test.ts — 0 discrepancies.
    return { r1: ((rPlus - r0) >>> D) | 0, r0 };
  };
  ...
  const internal: TRet<DSAInternal> = Object.freeze({
    ...
    verify: (
      ...
      // LOCAL MODIFICATION (F204-10). Upstream's verify held no `cleanBytes` call at all —
      // all 14 in the file were in keygen and signing. FIPS 204 §3.6.3 extends the duty
      // explicitly to this path: "The data used internally by verification algorithms is
      // similarly sensitive for some applications, including the verification of signatures
      // that are used as bearer tokens ... Intermediate values of the verification algorithm
      // may reveal information about its inputs".
      //
      // The bindings are hoisted so a `finally` can reach them: verify has seven early
      // `return false` exits, and zeroization that only runs on the success path would miss
      // exactly the cases an attacker drives. Same shape as `encapsulate` in
      // ../ml-kem/ml-kem.ts, which already wraps its scratch state in try/finally.
      //
      // WHAT IS DELIBERATELY NOT WIPED, and why — `splitCoder.decode` returns *subarray
      // views* into its input for numeric segments and fresh objects only for coder
      // segments (see splitCoder in ../ml-kem/utils.ts):
      //   • `rho`    — a view into the caller's `publicKey`; wiping it corrupts their key.
      //   • `cTilde` — a view into the caller's `sig`; likewise.
      //   • `msg`, `sig`, `publicKey` — caller-owned inputs. FIPS 204 §3.6.3 governs
      //     *intermediate* data; FIPS 205 §3.1 is the standard that extends this to "local
      //     copies of the inputs", and this is not that algorithm.
      //   • `mu` when `externalMu` is set — it *is* `msg`, aliased, not a fresh digest.
      // `t1`, `z` and `h` are fresh (vecCoder/hintCoder allocate), so they are wiped.
      let t1: Poly[] | undefined;
      let tr: Uint8Array | undefined;
      let mu: Uint8Array | undefined;
      let ownMu = false;
      let z: Poly[] | undefined;
      let h: Poly[] | false | undefined;
      let c: Poly | undefined;
      let zNtt: Poly[] | undefined;
      let c2: Uint8Array | undefined;
      const wTick1: Poly[] = [];
      try {
        // ML-DSA.Verify(pk, M, σ): Verifes a signature σ for a message M.
        const [rho, t1Decoded] = publicCoder.decode(publicKey); // (ρ, t1) ← pkDecode(pk)
        t1 = t1Decoded;
        tr = shake256(publicKey, { dkLen: TR_BYTES }); // 6: tr ← H(BytesToBits(pk), 512)
        ...
        const [cTilde, zDecoded, hDecoded] = sigCoder.decode(sig);
        z = zDecoded;
        h = hDecoded;
        ...
        c = crystals.NTT.encode(SampleInBall(cTilde)); // c ← SampleInBall(c˜1)
        zNtt = z.map((i) => i.slice()); // zNtt = NTT(z)
        ...
        for (let i = 0; i < K; i++) {
          ...
          // NOTE (F204-10): deliberately NO per-iteration cleanBytes here. These buffers
          // alias each other and the result: polySub mutates and returns `Az`, so
          // `wApprox === Az`, and polyUseHint is documented in-place on `u`, so the poly
          // pushed into `wTick1` is that same buffer. Wiping them here zeroes the
          // reconstructed commitment and every verification returns false. `wTick1` is
          // wiped in the finally block instead, which covers all three.
        }
        xof.clean();
        ...
        // LOCAL MODIFICATION (F204-10). Runs on every exit, including the seven early
        // `return false` paths and any throw.
        if (t1 !== undefined) cleanBytes(t1);
        if (tr !== undefined) cleanBytes(tr);
        if (ownMu && mu !== undefined) cleanBytes(mu);
        if (z !== undefined) cleanBytes(z);
        if (h !== undefined && h !== false) cleanBytes(h);
        if (c !== undefined) cleanBytes(c);
        if (zNtt !== undefined) cleanBytes(zNtt);
        if (c2 !== undefined) cleanBytes(c2);
        cleanBytes(wTick1);
      }
    },
  });
