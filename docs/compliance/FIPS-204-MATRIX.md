...
| 2026-09-09 | Adversarial verification pass found and corrected citation drift, all in `sign.ts`, caused by the F204-01 pass above widening that file's imports/JSDoc and shifting every line below: `MAX_CONTEXT_LENGTH` (`:6`→`:12`), the context throw block (`:18-23`→`:24-29`), the `sign`/provider call (`:45`→`:92`), `verify`'s try/call (`:68-73`→`:122-127`), the `sign` function range (`:39-46`→`:86-93`), `toNobleOptions` (`:14-25`→`:20-31`), and `algorithms.ts`'s `requireKey` (`:120-125`→`:169-174`, `:156`→`:153` in FIPS-203-MATRIX.md). F204-08's evidence table gained explicit line citations (`sign.test.ts:102`, `:132`), correcting a citation that read `:90` — the pre-existing signature-length test, not this fix's actual pk-length evidence. Two `sign.test.ts`/`sign-verify-catch.test.ts` line citations elsewhere corrected (`:111`→`:167`, `:31`→`:38`). `nist-vectors.test.ts:87` (and the co-cited `:78`) — structurally uncitable by line after the parametrized-loop refactor — recited by `describe` block name. §3.5's F204-10 scope line corrected: it named X-Wing, which has no verify/signature surface and is category-inapplicable to this finding (a copy-paste from FIPS-203-MATRIX's parallel F203-18 scope line). §3.9 corrected: it claimed "ML-DSA resolves to the npm package's own copy," false since the 2026-09-08 vendoring pass. No code changed; no row's status or determination changed. |
