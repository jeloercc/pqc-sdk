...
| **F204-05** | "While this value should ideally be generated by an approved RBG, other methods for generating fresh random values may be used."                                                                               | §3.6.1                    | SHOULD + MAY         | Applicable — the SDK signs in hedged mode only                                  | `sign.ts:92`; provider `sign` → `randomBytes(signRandBytes)` (`ml-dsa.js:453`)                             | `rnd` freshly generated per signature; approval is a recommendation, and `may` permits other fresh sources                                                                                                                                        | **CONFORMING** — the `may` clause is met; see §3.2                     |
| **F204-06** | "2: if 𝜉 = NULL then / 3: return ⊥ ▷ return an error indication if random bit generation failed"                                                                                                               | §5.1, Alg. 1              | Definitional         | Applicable                                                                      | `keys.ts` (`generate`, wraps `sampleRandomness`, maps `RbgFailureError` → `PqcError('RBG_FAILURE')`)       | RBG failure during key generation surfaces as `PqcError('RBG_FAILURE')` — `rbg-attribution.test.ts` (F204-06/07 suite)                                                                                                                            | **CONFORMING** — closed 2026-09-09 (corrective pass 2), see §3.3       |
| **F204-07** | "6: if 𝑟𝑛𝑑 = NULL then / 7: return ⊥ ▷ return an error indication if random bit generation failed"                                                                                                             | §5.2, Alg. 2 (and Alg. 4) | Definitional         | Applicable for Alg. 2; Alg. 4 not exposed                                       | `sign.ts` (`sign`, draws `rnd` via `sampleRandomness`, maps `RbgFailureError` → `PqcError('RBG_FAILURE')`) | RBG failure during signing surfaces as `PqcError('RBG_FAILURE')` — `rbg-attribution.test.ts` (F204-06/07 suite)                                                                                                                                   | **CONFORMING** — closed 2026-09-09 (corrective pass 2), see §3.3       |
...
| **F204-09** | "implementations of ML-DSA shall ensure that any potentially sensitive intermediate data is destroyed as soon as it is no longer needed."                                                                      | §3.6.3                    | SHALL                | Applicable                                                                      | Provider `keygen` / `sign` internals (14 `cleanBytes` sites, `ml-dsa.js`)                                  | Zeroization of expanded private-key polynomials, `rnd`, and signing scratch state. `provider-tripwires.test.ts` adds adjacent regression evidence (fresh keygens, fresh hedged signatures, non-aliasing outputs) but does not observe zeroization | **CONFORMING (delegated)** — see §3.5                                  |
...
| **F204-14** | "the lack of randomness in the deterministic variant makes the risk of side-channel attacks (particularly fault attacks) more difficult to mitigate. Therefore, this variant should not be used…"              | §3.4                      | SHOULD NOT           | **Not applicable** — deterministic variant is unreachable                       | `sign.ts:20-31` (`toNobleOptions` rebuilds the options object)                                             | —                                                                                                                                                                                                                                                 | **NOT APPLICABLE** — see §3.7                                          |
| **F204-15** | "Only implementing the hedged variant (i.e., without the deterministic variant) is sufficient to guarantee interoperability."                                                                                  | §3.4                      | Permissive           | Applicable                                                                      | `sign.ts:86-93` (`sign`)                                                                                   | Hedged-only exposure is a complete implementation; ACVP sigVer vectors verify externally produced signatures                                                                                                                                      | **CONFORMING** — see §3.7                                              |
| **F204-16** | "1: if \|𝑐𝑡𝑥\| > 255 then / 2: return ⊥ ▷ return an error indication if the context string is too long"                                                                                                        | §5.2, Alg. 2              | Definitional         | Applicable — the SDK exposes an optional context                                | `sign.ts:12` (`MAX_CONTEXT_LENGTH`), `sign.ts:24-29`, `sign.ts:124`                                        | Contexts over 255 bytes refused on **both** sign and verify, with a documented `PqcError`                                                                                                                                                         | **CONFORMING** — see §3.8                                              |
...
| **F204-21** | "This standard makes use of the functions SHAKE256 and SHAKE128, as defined in FIPS 202" + "This standard will always call these functions with an output length of a multiple of eight bits"                  | §3.7                      | Definitional         | Applicable                                                                      | Provider `ml-dsa.js` → `@noble/hashes/sha3.js` (`shake256`, `shake128`)                                    | `provider-tripwires.test.ts`: SHAKE256/SHAKE128 cross-checked against NIST FIPS 202 known-answer vectors (empty-string inputs, 32-byte output)                                                                                                    | **CONFORMING** — closed 2026-09-09 (corrective pass 2), see §3.10      |
...
(`sign.ts:92`) have no equivalent wrapper.
...
**The signature half conforms.** `sign.ts:126-127` calls the provider inside the try at
`sign.ts:125`, and `ml-dsa.js:590` does `if (sig.length !== sigCoder.bytesLen) return false;` —
...
`requireKey` (`algorithms.ts:169-174`) throws `PqcError('INVALID_KEY')` on a length mismatch.
...
**Objective evidence.** `sign.test.ts`, two new cases, both passing. Cited by line
explicitly here because the two are easy to conflate: `:90` (§3.4 above) is a
_pre-existing_, already-conforming signature-length behavior, not this fix's evidence —
the actual proof of F204-08's pk-length guard is `:102`, corrected here from an earlier
draft that miscited it as `:90`.

| Test name                                                                                | What it establishes                                                                                                                                                     |
| ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sign.test.ts:102` (`returns false (never throws) for a public key of the wrong length`) | The pk half of §3.6.2, at five lengths: 0, 1951, 1953, 1312 (a valid ML-DSA-44 pk) and 2592 (a valid ML-DSA-87 pk)                                                      |
| `sign.test.ts:132` (`still throws for a malformed key that is not a length mismatch`)    | The carve-out did not widen: wrong use still throws `WRONG_KEY_USE`, unknown algorithm still throws `UNSUPPORTED_ALGORITHM`, and a correct key still verifies to `true` |
...
`sign.test.ts:167` (`rejects verify with a non-ML-DSA public key`) still asserts a _throw_,
correctly, for a different condition (wrong algorithm, which §3.6.2 does not govern), and
`sign-verify-catch.test.ts:38` still uses a **correct** 1952-byte key on purpose so it reaches
the signer.
...
_Corrective pass 2 note._ `provider-tripwires.test.ts` asserts that repeated keygens and
hedged signatures produce fresh outputs and that keygen outputs do not alias. Those are
useful regression tripwires, but none of them would fail if a `cleanBytes` call were
deleted — JavaScript gives no way to observe freed intermediates from a test. The row
therefore stays **CONFORMING (delegated)**. Since corrective pass 2, `sign()` supplies
`rnd` itself, so the vendored signer no longer wipes it; `sign.ts` zeroes it in a
...
**Scope.** Closed for ML-DSA. SLH-DSA still resolves against the npm package and remains
provider-scope for this requirement. X-Wing is not mentioned here — corrected 2026-09-09:
F204-10 is about zeroization in ML-DSA's `internal.verify`, and X-Wing is a KEM with no
`verify`/signature surface at all, so this requirement does not apply to it in any scope.
The earlier text naming X-Wing here was a copy-paste from the FIPS-203-MATRIX's parallel
F203-18 scope line, unadjusted for the fact that this finding is category-inapplicable to
X-Wing.
...
throughout (`nist-vectors.test.ts`'s `describe('NIST ACVP ML-DSA-65 keyGen (FIPS 204)', ...)`
and `describe('NIST ACVP ML-DSA-65 sigVer (FIPS 204, pure)', ...)` blocks — cited by name,
not line number, since 2026-09-09's parametrized-loop refactor made every ML-DSA block's
line position shift with which set it's iterating) and no functional deviation was ever
claimed.
...
2. `toNobleOptions` (`sign.ts:20-31`) does not forward its argument. It **constructs a new
   ...
   reached.
3. `sign.ts:92` passes that constructed object as the only options argument.
...
produced signatures is evidenced by the ACVP sigVer vectors in `nist-vectors.test.ts`'s
`describe('NIST ACVP ML-DSA-65 sigVer (FIPS 204, pure)', ...)` block (cited by name, not
line number — see the F204-13 note above on why), which verify signatures this SDK did not
generate.

### 3.8 F204-16 — Context string

**CONFORMING.** `sign.ts:12` sets `MAX_CONTEXT_LENGTH = 255`, citing FIPS 204 §5.2.
`toNobleOptions` (`sign.ts:24-29`) throws `PqcError('INVALID_CONTEXT')` above that length, and
`sign.ts:124` deliberately calls it **outside** the try in `verify` — the inverse of the
F204-08 defect, and correct here, since an oversized context is a caller error rather than a
signature that should verify to `false`. The comment at `sign.ts:122-123` records that intent.
...
- A repository-wide grep over `packages/core/src` and `packages/cli/src` finds **no** reference
  ...
  vendored `utils.ts` is shared by both the vendored ML-KEM and ML-DSA modules (`ml-dsa.ts`
  imports its `Signer`/`CryptoKeys` types and helpers from it), and neither vendored module's
  own file exposes a `prehash` export — corrected 2026-09-09: this previously said "ML-DSA
  resolves to the npm package's own copy," which was already false by the time it was
  written (ML-DSA was vendored into `packages/core/src/vendor/ml-dsa/ml-dsa.ts` on
  2026-09-08, §3.5/§3.6) and is more false now that ML-DSA-44/87 are registered against
  that same vendored copy too (§3.1).
- `sign.ts:92` calls `spec.signer.sign(...)` — the pure variant only.
...
Corrective pass 2, 2026-09-09: F204-06, F204-07, F204-21 closed; F204-05 evidence added;
F204-09 stays delegated (§3.5). All 22 rows determined; CONFORMING (delegated) count
reduced from 4 to 1.

| Status                                            | Count  | Req IDs                                                                                           |
| ------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------- |
| CONFORMING                                        | 11     | F204-01, F204-05, F204-06, F204-07, F204-08, F204-10, F204-11, F204-13, F204-15, F204-16, F204-21 |
| CONFORMING (delegated)                            | 1      | F204-09                                                                                           |
...
- **F204-10 and F204-13 were provider-scope** when first recorded. Both are now CONFORMING
  because ML-DSA was vendored into `src/vendor/ml-dsa/` (2026-09-08). This note is retained
  as institutional memory of the constraint that drove that decision.
...
- **F204-06, F204-07, F204-21 are no longer delegated** (corrective pass 2, 2026-09-09).
  F204-06/07 are now evidenced by RBG-failure tests in `rbg-attribution.test.ts`; F204-21 by
  FIPS 202 known-answer tests in `provider-tripwires.test.ts`. **F204-09 remains
  `CONFORMING (delegated)`**: zeroization is not observable from a JavaScript test, and the
  behavioral tests added for it guard adjacent properties only (§3.5).
...
1. ~~**F204-08**, **F204-10**, **F204-13**~~ — closed 2026-09-08 (§3.4, §3.5, §3.6). No
   further action.
2. ~~**F204-06, F204-07**~~ — closed 2026-09-09 (corrective pass 2): `keys.ts` and `sign.ts`
   now draw randomness via `sampleRandomness` and map `RbgFailureError` →
   `PqcError('RBG_FAILURE')` for every failure mode (throw, missing API, short buffer);
   tested in `rbg-attribution.test.ts`.
   No further action.
3. ~~**F204-05**~~ — closed 2026-09-09 (corrective pass 2): `sign.test.ts` asserts two
   signatures over the same message differ. No further action.
4. ~~**F204-21**~~ — closed 2026-09-09 (corrective pass 2): `provider-tripwires.test.ts`
   checks SHAKE256/SHAKE128 against FIPS 202 known-answer vectors. **F204-09** stays
   delegated; the same file adds adjacent regression tripwires, not zeroization evidence.
...
| 2026-09-09 | Adversarial verification pass. Citation drift corrected in `sign.ts` (all line numbers shifted by the F204-01 refactor); F204-08 evidence table corrected; §3.5 and §3.9 narrative corrected. No code changed; no row status changed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 2026-09-09 | Corrective pass 2. Closed three open rows (F204-09 reviewed, left delegated): **F204-06** — `keys.ts` `generate()` wraps `sampleRandomness` and maps `RbgFailureError` → `PqcError('RBG_FAILURE')`; tested in `rbg-attribution.test.ts` (F204-06/07 suite). **F204-07** — `sign.ts` `sign()` draws `rnd` via `sampleRandomness` (passed as `extraEntropy`, zeroed afterwards) and maps `RbgFailureError` → `PqcError('RBG_FAILURE')`; same suite. **F204-09** — not closed: `provider-tripwires.test.ts` adds adjacent regression tripwires (distinct keygens, distinct hedged signatures, non-aliasing outputs), which cannot observe zeroization. **F204-21** — same file cross-checks SHAKE256/SHAKE128 against NIST FIPS 202 known-answer vectors. Additionally: **F204-05** evidence added to `sign.test.ts` (two signatures over same message differ). CONFORMING (delegated) count reduced from 4 to 1. Summary table updated.                                                                                                                                                                                                                                        |
