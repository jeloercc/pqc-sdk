...
**Update, 2026-09-09 (corrective pass 1).** X-Wing's embedded ML-KEM-768 component now resolves to the
...
unchanged by that work — still **INDETERMINATE** at the time of that entry, for the
same segregation reason recorded here. Correcting the component ML-KEM-768's arithmetic
method does not change what X-Wing _is_; X-Wing counted for nothing toward FIPS 203
coverage before this change and counts for nothing toward it now.

**Update, 2026-09-09 (corrective pass 2).** The FIPS-approved / draft boundary is now
expressed in the public API. `index.ts` exports `FIPS_ALGORITHMS`, a new constant
listing only `['ml-kem-768', 'ml-dsa-44', 'ml-dsa-65', 'ml-dsa-87']` — explicitly
excluding `x-wing` — with JSDoc citing FIPS 203 §3.3 and this matrix §3.2 to make
the distinction discoverable. `SUPPORTED_ALGORITHMS` continues to include `x-wing` for
feature completeness; `FIPS_ALGORITHMS` is the compliance-safe subset. A caller reading
the public surface now has an explicit machine-checkable boundary between FIPS-covered
and draft entries. **Status: CONFORMING.** The `should`-level duty (FIPS 203 §3.3) is
met: the assessment has been made and its conclusion is expressed in the API surface.
...
**Update, 2026-09-09 (corrective pass 2).** Both gaps are now closed by direct tests.

| Check                               | Repository evidence                                                                                                           |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| §7.2 step 1 — ek type check         | `encrypt.test.ts:58` — wrong-length key → `INVALID_KEY`                                                                       |
| §7.2 step 2 — ek modulus check      | `fips203-input-checks.test.ts` — coefficient `q` injected into ek, `encapsulate` throws                                       |
...
| §7.3 step 3 — dk hash check         | `fips203-input-checks.test.ts` — stored `H(ek)` bit-flipped, `decapsulate` throws; hash independently re-derived and compared |

The bit-flip matrix in `key-mutations.test.ts:97-140` still does not fill these gaps
(the property under test is different from the property claimed), but the dedicated
tests above do: each targets exactly the normative check, not an observable side-effect
of it. A version bump that silently removed either check would now fail CI.

### 3.4 F203-11 — RBG failure misattributed as an invalid encapsulation key

**Status: CLOSED 2026-09-07 for the standalone `ml-kem-768` algorithm entry. That closure
did not extend to X-Wing's default path — a differently-shaped gap than §3.10's, found and
closed 2026-09-09; see §3.11 for the full account. Severity when open: low (diagnostic
integrity). Never exploitable; not a key-recovery or forgery issue.**
...
10 cases (9 from this closure, 1 added 2026-09-09 — see §3.11), all passing:
...
| `reports RBG_FAILURE through the public pqc.encrypt entry point too`                 | The code reaches real callers — but only via the `ml-kem-768` argument this test pins; see §3.11 for why that pin let the X-Wing gap through          |
| `reports RBG_FAILURE through the true default path — no algorithm argument anywhere` | Added 2026-09-09 (§3.11): `pqc.keys.generate()` with no options, then `pqc.encrypt` with no algorithm argument — the actual default, not a pinned one |
...
The last three of the original nine are the ones that matter for review: the obvious way
to get this wrong is to widen the new branch until it captures cases that genuinely _are_
invalid keys. The new tenth case matters for a different reason — it exercises the
argument nobody pins.
...
### 3.11 F203-11 — a second, differently-shaped gap in X-Wing's default path

**Status: gap existed since F203-11's original closure (2026-09-07) through 2026-09-09;
found and closed 2026-09-09.**

§3.10 closed the X-Wing gap for F203-19 and F203-18 by substituting the vendored
`ml_kem768` for the npm-internal one inside `x-wing.ts`'s composition. An independent
adversarial verification pass then asked whether F203-11 (this row) had the same shape of
gap, and it did — but not the same _cause_, which matters for what the fix could be.

**The call site, found by reproducing the failure, not by inspection.** Stubbing
`crypto.getRandomValues` to throw and calling `x-wing.ts`'s own (pre-fix)
`ml_kem768_x25519.encapsulate(publicKey)` — no seed argument, the shape every real caller
uses — threw a raw `Error`, with this stack:

```
Error: RBG STUB FAILURE
    at randomBytes (@noble/hashes/utils.js:669:15)
    at Object.encapsulate (@noble/post-quantum/hybrid.js:470:38)
```

`hybrid.js:470` is `combineKEMS`'s own returned `encapsulate` method:
`encapsulate(pk, randomness = randomBytes(msgCoder.bytesLen)) { ... }`. This default
parameter is generic to `combineKEMS` itself — it samples the _combined_ top-level
randomness before splitting it across component KEMs, using `@noble/post-quantum`'s
plain, unpatched `randomBytes`. **This is why the §3.10 fix model — substitute the
vendored component — does not reach this one.** The failing call is not inside
`ml_kem768` at all; substituting a corrected `ml_kem768` changes nothing about
`combineKEMS`'s own randomness sampling, which runs before either component KEM is
invoked.

**Is it closable at proportionate cost? Yes — verified before assuming so.** The
alternative would be vendoring `combineKEMS` itself to patch its internal default
parameter — exactly the cost §3.10 already declined to pay for `combineKEMS`,
`expandSeedXof` and `_ecdhKem`, since none of them perform ML-KEM arithmetic. That
vendoring is not necessary here either: `x-wing.ts` does not have to accept
`combineKEMS`'s object as-is. It can (and now does) export a wrapper whose `encapsulate`
always supplies an explicit `randomness` argument to the composed object — sampled via
`vendor/ml-kem/ml-kem.ts`'s `sampleRandomness` (the exact F203-11 correction, exported for
this reuse) when the caller passes none, forwarded unchanged when the caller does. Because
the wrapper's argument is always present, `hybrid.js:470`'s default parameter — the actual
defect — is never evaluated. The fix is one layer of interception above the gap, not a
vendored copy of it.

**Objective evidence.** `packages/core/src/vendor/ml-kem/__tests__/rbg-attribution.test.ts`
gains a tenth case, `reports RBG_FAILURE through the true default path — no algorithm
argument anywhere`: `pqc.keys.generate()` with no options (asserted to be `'x-wing'`),
then `pqc.encrypt(...)` with no algorithm argument anywhere, against a stubbed RBG,
asserting `RBG_FAILURE`. This is a deliberate correction to the existing evidence, not
just an addition: the prior sole public-API case (`reports RBG_FAILURE through the public
pqc.encrypt entry point too`) pins `{ algorithm: 'ml-kem-768' }`, which is exactly why it
could not see this gap — it never exercised the actual default. Reverted and reran to
confirm the new case is a genuine regression guard, not a tautology: with the fix in
`x-wing.ts` stashed out, this one case fails (`INVALID_KEY` instead of `RBG_FAILURE`) while
every other test in the file and the rest of the suite stays green; restored, all 10 cases
in the file and 355 tests repo-wide pass. `xwing-vectors.test.ts`'s 18 KAT cases (seeded,
deterministic encapsulation) also re-verified passing, confirming the wrapper forwards an
explicit seed untouched and only intervenes when the caller supplies none.

**Adjacent gap, deliberately not fixed here, recorded so it does not disappear.**
`keys.ts`'s own top-level `randomBytes(spec.seedLength)` — the call `pqc.keys.generate()`
makes for _every_ algorithm before ever reaching a KEM or signer's own `keygen` — is
itself unwrapped against RBG failure; a host RBG failure there surfaces as a raw `Error`,
not `RBG_FAILURE`, for `ml-kem-768`, `x-wing`, and every ML-DSA set alike. This is **not**
part of F203-11's scope: F203-11 quotes FIPS 203 Algorithm 20 (`ML-KEM.Encaps`), steps 2–4
— a condition defined _inside_ encapsulation, not key generation. `combineKeys`'s own
parallel "no seed" fallback inside `combineKEMS` (the same shape as the bug this section
just closed) is similarly not reachable via the public API for the same reason noted in
§3.10-adjacent analysis: `keys.ts` always supplies an explicit seed to
`spec.kem.keygen(seed)`/`spec.signer.keygen(seed)`, so that particular internal branch
never fires — but `keys.ts`'s own sampling, one level up, is not itself guarded. Recorded
here rather than silently dropped because it is real, and because "out of scope for this
row" must not read as "resolved."

---

## 4. Summary

As of the 2026-09-09 corrective pass 2 (F203-03, F203-05, F203-10 closed):

| Status                                           | Count  | Req IDs                                                                                                    |
| ------------------------------------------------ | ------ | ---------------------------------------------------------------------------------------------------------- |
| CONFORMING                                       | 12     | F203-02, F203-03, F203-04, F203-05, F203-07, F203-08, F203-09, F203-10, F203-11, F203-16, F203-17, F203-19 |
| CONFORMING (delegated)                           | 2      | F203-06, F203-15                                                                                           |
...
| NONCONFORMING                                    | 0      | —                                                                                                          |
| INDETERMINATE                                    | 2      | F203-12, F203-13                                                                                           |
...
2026-09-08 corrective pass — CONFORMING 9, CONFORMING (delegated) 4, INDETERMINATE 3.
2026-09-09 corrective pass 1 — counts unchanged (F203-03 still INDETERMINATE).
2026-09-09 corrective pass 2 — CONFORMING 12, CONFORMING (delegated) 2, INDETERMINATE 2.

**The NONCONFORMING count reaching zero does not mean the SDK is FIPS 203 compliant, and
this table must not be cited to that effect.** F203-18 carries a qualifier recording a
limitation no change to this repository can remove. F203-12 remains INDETERMINATE
because it cannot be settled from inside a library. Read §4.1 before quoting any number
from this table.
...
- **F203-05 and F203-10** are now evidenced by dedicated tests, not source inspection alone.
  `fips203-input-checks.test.ts` exercises the modulus check and the hash check directly;
  a version bump that removes either check now fails CI (corrective pass 2, 2026-09-09).
- **F203-03** is now CONFORMING. `FIPS_ALGORITHMS` in `index.ts` provides a machine-checkable
  FIPS-only subset, and its JSDoc cites the standard explicitly.
...
approved RBG — is the one that cannot be settled here at all. One row remains unresolved
on keyword interpretation (F203-14), and one parameter set of three is exposed (F203-01).
